...
oledBootLogo = "1306boot.bmp"
oledShutdownLogo = "1306shutdown.bmp"
    ...
    def DrawOn(self, image):
        if oled.duration != None:
          ...
          for i in range(0, len(data)-1):
              try:
                  self.draw.rectangle((i*oledspecWide1, oledspecYposTag, i*oledspecWide1+oledspecWide2, oledspecYposTag-int(data[i])), outline = oledspecBorder, fill =oledspecFill) 
        ...
        if oled.duration == None:
          ...
          for i in range(0, len(data)-1):
              try:
                  self.draw.rectangle((i*oledspecWide1, oledspecYposNoTag, i*oledspecWide1+oledspecWide2, oledspecYposNoTag-int(data[i])), outline = oledspecBorder, fill =oledspecFill) 
                  image.paste(self.image, (0, 0))
                  if specstatus1 != specstatus2:
                       oled.activeSong = newSong
                       oled.activeArtist = newArtist
