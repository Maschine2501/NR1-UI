...
from modules.leds import delay, light_up_leds, turn_off_leds_after_delay, deactivate_play, deactivate_pause, deactivate_back, deactivate_forward, deactivate_shuffle, deactivate_repeat, deactivate_ButtonD, deactivate_ButtonC
from modules.volumiosocket import activate_play, activate_pause, activate_back, activate_forward, activate_ButtonC, activate_ButtonD
...
def main():
    ...
    # Wait for Volumio to start up
    time.sleep(10)

    # Light up the LEDs after volumio has started up
    light_up_leds(mcp23017)
    turn_off_leds(mcp23017)  # Turn off LEDs after lighting them up

    monitor_volumio_state_thread = threading.Thread(target=monitor_volumio_state)
    monitor_volumio_state_thread.daemon = True
    monitor_volumio_state_thread.start()
