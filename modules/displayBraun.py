    ...
    def DrawOn(self, image, position):
        if self.center:
            ...
            if self.textwidth < width:
                position = (int((width-self.textwidth)/2 + 36), position[1])   #original -> "position = (int((width-self.textwidth)/2 + 42), position[1])  "
...
class ScrollText(Screen):
    ...
    def DrawOn(self, image, position):
        ...
        if self.textwidth <= width:                  # center text
            position = (int(((width-self.textwidth)/2)+36), position[1])     #original-> "position = (int((width-self.textwidth)/2), position[1])"
